<?php
/**
 * Saisie d'écriture — version réécrite.
 */
...
$page_title = 'Nouvelle Écriture (version réécrite)';
...
if (is_post() && !$entry) {
    ...
    if (empty($errors)) {
        ...
        audit_log('CREATE', 'entries', $entry_id, "Entry created (cible): $piece_number");
        set_flash('success', "Écriture enregistrée : $piece_number");
        redirect('/modules/entries/edit_cible.php?id=' . $entry_id);
    } else {
...
<h2 data-testid="page-title"><?php echo $page_title; ?></h2>
...
<div class="mt-20">
    ...
    <a href="/modules/entries/edit_cible.php" class="btn btn-primary">Nouvelle écriture</a>
</div>

<?php else: ?>

<form method="post" action="" id="entry-form" data-testid="entry-form">
    <?php echo csrf_field(); ?>

    <div class="form-row mb-20">
        <div class="form-group">
            <label for="entry_date">Date *</label>
            <input type="date" id="entry_date" name="entry_date" data-testid="entry-date" value="<?php echo date('Y-m-d'); ?>" required>
        </div>
        <div class="form-group">
            <label for="journal_id">Journal *</label>
            <select id="journal_id" name="journal_id" data-testid="journal" required>
        ...
        <div class="form-group" style="flex: 2;">
            ...
            <input type="text" id="label" name="label" data-testid="label" style="width: 100%;" required>
        </div>
    </div>
    ...
    <div class="mt-10">
        <button type="button" class="btn btn-add-line" data-testid="add-line">+ Ajouter ligne</button>
    </div>
    ...
    <div class="mt-20">
        <button type="submit" class="btn btn-success" data-testid="save">Enregistrer</button>
