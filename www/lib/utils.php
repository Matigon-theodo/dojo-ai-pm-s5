...
/**
 * Generate next lettering code for an account (A, B, ..., Z, AA, AB, ...)
 * Codes are sequential per account (compte de tiers).
 */
function next_lettrage_code($account_id) {
    $account_id = intval($account_id);

    // Find the highest code currently used on this account
    $sql = "SELECT lettrage FROM entry_lines
            WHERE account_id = $account_id AND lettrage IS NOT NULL AND lettrage <> ''";
    $rows = db_fetch_all(db_query($sql));

    $max = '';
    foreach ($rows as $r) {
        $code = strtoupper(trim($r['lettrage']));
        // Longer code sorts higher (Z < AA); same length compares alphabetically
        if (strlen($code) > strlen($max) || (strlen($code) === strlen($max) && $code > $max)) {
            $max = $code;
        }
    }
    ...
    // Increment like a spreadsheet column: Z -> AA, AZ -> BA, ...
    $chars = str_split($max);
    $i = count($chars) - 1;
    while ($i >= 0) {
        if ($chars[$i] === 'Z') {
    ...
    // All positions wrapped (e.g. ZZ -> AAA)
